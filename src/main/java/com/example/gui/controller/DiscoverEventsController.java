...
public class DiscoverEventsController implements EventHandler<ActionEvent> {
    ...
    public void handle(ActionEvent event) {
        ...
        } else if (event.getSource() == participateButton) {
            Iterable<Event> events = service.printAllEvents();
            List<Event> allEvents1 = new ArrayList<>();
            for (Event event1 : events) {
                if (!event1.getCreator().equals(service.getCurrentUserId())) {
                    allEvents1.add(event1);
                }
            }
            if (participateButton.getText().equals("Follow!")) {
                service.saveParticipant(allEvents1.get(currentPage).getId());
                participateButton.setText("Unfollow!");
            } else if (participateButton.getText().equals("Unfollow!")) {
                service.deleteParticipant(allEvents1.get(currentPage).getId());
                participateButton.setText("Follow!");
            }
            initialize();
        ...
        } else if (event.getSource() == eventsButton || event.getSource() == yourEventsButton) {
            try {
    ...
    private void initialize() {
        ...
        if (allEvents.size() != 0) {
            if (service.verifyParticipant(allEvents.get(currentPage), service.getCurrentUserId())) {
                participateButton.setText("Unfollow!");
            } else {
                participateButton.setText("Follow!");
            }
